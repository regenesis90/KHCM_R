#' capacity of people in the Bus stop stoppinglot(c_p_b)
#'
...
#' @param P Number of passenger by bus
#' @export c_p_b
#' @examples
c_p_b <- function(flow_type = NULL, R = NULL, t_c = NULL, t_D = NULL, g_c_ratio = NULL, N = NULL, P = NULL){
  if (flow_type == 'continuous'){c <- 3600 * R * N * P / (t_c + t_D)}
  if (flow_type == 'interrupted'){c <- g_c_ratio * 3600 * R * N * P / (t_c + g_c_ratio * t_D)}
  c
}
