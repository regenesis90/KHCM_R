...
### 8. 신호교차로(si)
* `LOS_si` : 신호교차로의 서비스 수준
* `S_0_si` : 신호교차로 기본 조건에서 포화교통류율
* `V_P_si` : 신호교차로에서 첨두시간 교통류율(vph)
* `V_si` : 신호교차로에서 차로이용률을 보정한 첨두시간 교통량(vph)
* `F_U_si` : 신호교차로에서 교통량의 차로이용률 보정계수
* `F_R_si` : 신호교차로에서 우회전 교통량 보정계수
* `V_R_si` : 신호교차로에서 적색신호에 우회전하는 차량(RTOR)을 제외하여 보정한 우회전 교통량(vph)
* `E_L_si` : 신호교차로에서 좌회전 차로의 종합 직진환산계수
* `P_si` : 신호교차로에서 대향직진 교통량별 한 gap당 비보호좌회전 가능 대수
* `E_1_si` : 신호교차로에서 좌회전 자체의 직진환산계수
* `E_p_si` : 신호교차로에서 좌회전 곡선반경 영향 직진환산계수
* `E_u_si` : 신호교차로에서 U턴%에 따른 좌회전의 직진환산계수
* `L_dw_si` : 신호교차로에서 진출입차량의 방해 시간. 이면도로 진출입으로 인한 시간당 손실시간(초)
* `T_b_si` : 신호교차로에서 버스 1대의 정차에 따른 포화차두시간 손실시간(초)
* `L_bb_si` : 신호교차로에서 진출입차량의 방해 시간. 이면도로 진출입으로 인한 시간당 손실시간(초)
* `L_p_si` : 신호교차로에서 주차활동으로 인한 방해시간(초)
* `L_H_si` : 신호교차로에서 측 차로의 노변마찰에 의한 종합 차두시간 손실(초)
* `S_R0_si` : 신호교차로에서 우회전의 포화교통류율
* `E_R_si` : 신호교차로 우회전 차로에서 공용우회전의 직진환산계수
* `f_c_si` : 신호교차로에서 우회전 차량이 이용할 수 없는 횡단보도 신호시간 비율
* `V_RF_si` : 신호교차로에서 공용 우회전 차로에서 첫 우회전 앞에 도착하는 직진 교통량(vph) 
* `V_LF_si` : 신호교차로에서 공용 좌회전 차로에서 첫 좌회전 앞에 도착하는 직진 교통량(vph) 
* `V_STL_si` : 신호교차로에서 공용 좌회전 차로를 이용하는 직진 교통량(vph)
* `V_STR_si` : 신호교차로에서 공용 우회전 차로를 이용하는 직진 교통량(vph)
* `P_L_pd_si` : 신호교차로 실질적 전용 좌회전 차로군 i의 회전 교통량비
* `P_R_pd_si` : 신호교차로 실질적 전용 우회전 차로군 i의 회전 교통량비
* `P_LT_sh_si` : 신호교차로 공용 좌회전 차로군 i의 회전 교통량비, 직진-좌회전 공용차로군에서 좌회전의 비율
* `P_RT_sh_si` : 신호교차로 공용 우회전 차로군 i의 회전 교통량비. 직진 + 우회전 공용차로군에서 우회전의 비율
* `V_T_int_si` : 신호교차로 직진, 좌회전, 우회전 통합 차로군에서 접근로의 총교통량(vph)
* `P_LT_int_si` : 신호교차로 직진+좌+우회전 통합차로군의 좌회전 교통량비
* `P_RT_int_si` 신호교차로 직진+좌+우회전 통합차로군의 우회전 교통량비
* `f_L_d_si` : 신호교차로 전용 좌회전 차로군의 좌회전 보정계수
* `f_L_pd_si` : 신호교차로 실질적 전용 좌회전 차로군의 좌회전 보정계수
* `f_R_pd_si` : 신호교차로 실질적 전용 우회전 차로군의 우회전 보정계수
* `f_LT_sh_si` : 신호교차로 공용 좌회전 차로군의 좌회전 보정계수
* `f_RT_sh_si` : 신호교차로 공용 우회전 차로군의 우회전 보정계수
* `f_int_si` : 신호교차로 통합차로군의 좌회전, 우회전 보정계수
* `S_bi`
* `f_BLT`
* `S_bi`
* `f_ub`
...
* `capa_i_di` : 2점 교차형 다이아몬드형 인터체인지(고속도로 연결로-일반도로 결합부)에서, i차로군의 포화도(교통량과 용량의 비)
* `X_i_di` : 2점 교차형 다이아몬드형 인터체인지(고속도로 연결로-일반도로 결합부)에서, i차로군의 포화도
* `d_E_di` : 2점 교차형 다이아몬드형 인터체인지(고속도로 연결로-일반도로 결합부)에서, 교차로1을 직진 통과하여 교차로2에서 좌회전하는 이동류 E의 지체
* `d_total_di` : 2점 교차형 다이아몬드형 인터체인지(고속도로 연결로-일반도로 결합부) 전체의 차량 당 평균제어지체(sec/veh)
### 10. 비신호교차로(nsi)
